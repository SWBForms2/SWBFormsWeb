...
"0")+Math.abs(b)+":00"),Time.adjustForDST=!1,NumberUtil.decimalSymbol=".",NumberUtil.groupingSymbol=",",Offline.explicitOffline=!1,eng.utils.loadJS(eng.contextPath+"/platform/js/eng_lang.min.js",!1,c,!0,eng.staticVersion),isc.RPCManager.addClassProperties({handleError:function(a,b){if(a.data)if("string"===typeof a.data)isc.warn(a.data);else if("object"===typeof a.data){b=window[a.context.componentId];var c=a.data._errors,e=a.data._hints,f=a.data._msg,g=a.data._enabled,k=a.data._visible;delete a.data._errors;
delete a.data._hints;delete a.data._msg;delete a.data._enabled;delete a.data._visible;for(var h in a.data)b.setValue(h,a.data[h]);if(c){a="";for(h in c)b.setFieldErrors(h,c[h]),a+="<b>"+b.getItem(h).title+"</b>: "+c[h]+"<br>",b.setFocusItem(h);f||(f=a)}if(e)for(h in c)b.getItem(h).setHint(e[h]);if(g)for(h in g)b.getItem(h).setDisabled(!g[h]);if(k)for(h in k)k[h]?b.getItem(h).show():b.getItem(h).hide();b.redraw();f&&isc.warn(f);for(h in c){b.setFocus(h);break}}return!1}}));if("string"===typeof a)"/"!=
a.charAt(0)?eng.dataSourceScriptPath=window.location.pathname.substring(0,window.location.pathname.lastIndexOf("/"))+"/"+a:eng.dataSourceScriptPath=a,eng.utils.loadJS(eng.contextPath+a,!1,c);else if(Array.isArray(a))for(eng.dataSourceScriptPath=window.location.pathname.substring(0,window.location.pathname.lastIndexOf("/"))+"/"+JSON.stringify(a),n=0;n<a.length;n++)eng.utils.loadJS(eng.contextPath+a[n],!1,c);window.console.warn=function(){};window.onbeforeunload=function(){return eng.onbeforeunload()}}},

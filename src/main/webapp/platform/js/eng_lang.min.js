...
if(record)if("function"==typeof dsfmt)return dsfmt(value,record);else return eval(dsfmt);else return value};base.optionDataSource=dsObjDef.dsId;if(!base.editorProperties)base.editorProperties={};if(!base.editorProperties.sortField)if(base.editorProperties.sortField===null)delete base.editorProperties.sortField;else base.editorProperties.sortField=dsf;if(base.textMatchStyle)base.editorProperties.textMatchStyle=base.textMatchStyle;else if(!base.editorProperties.textMatchStyle)base.editorProperties.textMatchStyle=
"substring";if(base.getFilterCriteria)base.editorProperties.getPickListFilterCriteria=eng.utils.removeAttribute(base,"getFilterCriteria");if(base.showFilter)base.pickListProperties={showFilterEditor:eng.utils.removeAttribute(base,"showFilter")};if(base.selectFields)base.pickListFields=eng.utils.removeAttribute(base,"selectFields");if(base.selectWidth)base.pickListWidth=eng.utils.removeAttribute(base,"selectWidth");if(base.initialCriteria)base.optionCriteria=eng.utils.removeAttribute(base,"initialCriteria");
return base};
...
eng.fieldProcesors["html"]=function(field){var base=eng.utils.cloneObject(field);if(!base.editorType)base.editorType="RichTextItem";if(!base.width)base.width="100%";if(!base.height)base.height="200";if(!base.startRow)base.startRow=true;if(!base.colSpan)base.colSpan=5;if(!base.showTitle)base.showTitle=true;return base};
...
if(i<dataValue.length-1)content+=",";content+="        </span>"}if(this.isEditable())content='<button onclick="'+this.ID+'_button.click()">Cargar Archivo</button>'+content;content='<div style="width:100%; height:18px;">'+content+"</div>";this.canvas.setContents(content);if(this.isEditable()&&!this.canvas.uploader)this.initUploader(this.canvas)},initUploader:function(canvas){console.log("initUploader:",this.ID);var div=document.createElement("div");div.innerHTML='<div id="'+this.ID+'_container" style="display:none">\n'+
